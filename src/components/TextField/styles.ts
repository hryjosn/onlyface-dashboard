...
import { rgbaWithAlpha } from "~/lib/legacy/theme/utils/rgbaWithAlpha"
...
export const TextFieldStyled = styled(TextField)<TextFieldProps>(
  ({ theme }) => ({
    ...
    "& .MuiInputBase-root": {
      ...
      border: `1px solid ${rgbaWithAlpha(theme.palette.customColors.main, 0.2)}`,
      transition: theme.transitions.create(["border-color", "box-shadow"], {
        duration: theme.transitions.duration.shorter,
      }),
      "&:not(.Mui-focused):not(.Mui-disabled):not(.Mui-error):hover": {
        borderColor: `${rgbaWithAlpha(theme.palette.customColors.main, 0.28)}`,
      },
      "&:before, &:after": {
        display: "none",
      },
